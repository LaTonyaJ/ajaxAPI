...
async function searchShows(query) {
  ...
  // hard coded data.
  const res = await axios.get(`http://api.tvmaze.com/search/shows?q=${query}`,{params:{username: 'latonya.johnson', password:'uBNPnbIZCmw_UQWNRn2LGMPcykoEtaHX'}});
  //console.log(res.data);
  //const data =  [{ show: { id, name, summary, image:{ original } } }]  = res.data;
  let shows = res.data.map(row => {
    let show = row.show; //row.show?
    return {id: show.id,
            name: show.name,
            summary: show.summary,
...
async function getEpisodes(id) {
  ...
  let episodes = res.data.map(episode => ({
    //let episode = row.episode; //row.show?
            id: episode.id,
            name: episode.name,
